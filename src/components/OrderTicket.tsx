  ...
  return (
    <div style={{ fontFamily: "'Courier New', Courier, monospace", fontSize: '13px', fontWeight: 'bold', width: '272px', lineHeight: '1.5', color: '#000' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '6px' }}>
        <div style={{ fontSize: '15px' }}>{d.orgName}</div>
        <div style={{ fontSize: '12px' }}>{d.branchName}</div>
        <div style={{ borderTop: '1px dashed #000', borderBottom: '1px dashed #000', padding: '2px 0', marginTop: '4px', fontSize: '12px' }}>
          TICKET DE VENTA
        </div>
      </div>
      ...
      <div style={{ fontSize: '12px' }}>
        <div style={{ borderBottom: '1px solid #000', paddingBottom: '2px', marginBottom: '3px' }}>
          {pad('Producto', 'Cant   Total')}
        </div>
        {d.items.map((item, i) => (
          <div key={i} style={{ marginBottom: '3px' }}>
            <div>{item.name}</div>
            <div style={{ fontSize: '11px' }}>{pad(`  ${cur}${item.unitPrice.toFixed(2)} c/u`, `${item.quantity}x  ${cur}${item.subtotal.toFixed(2)}`)}</div>
          </div>
        ))}
      </div>
      ...
      {d.notes && (
        <div style={{ fontSize: '11px', marginTop: '4px', fontStyle: 'italic' }}>Nota: {d.notes}</div>
      )}

      <div style={{ textAlign: 'center', fontSize: '11px', marginTop: '6px', borderTop: '1px dashed #000', paddingTop: '4px' }}>
  ...
  return (
    <div style={{ fontFamily: "'Courier New', Courier, monospace", fontSize: '14px', fontWeight: 'bold', width: '272px', lineHeight: '1.6', color: '#000' }}>
      ...
      <div>
        {d.items.map((item, i) => (
          <div key={i} style={{ marginBottom: '6px' }}>
            ...
            {item.notes && (
              <div style={{ fontSize: '13px', paddingLeft: '4px', borderLeft: '3px solid #000', marginTop: '2px' }}>
                ! {item.notes}
              </div>
            )}
          </div>
      ...
      {d.notes && (
        <>
          <div style={{ borderTop: '1px dashed #000', margin: '4px 0' }} />
          <div style={{ fontSize: '13px' }}>NOTA: {d.notes}</div>
        </>
      )}
  ...
  return (
    <div id="thermal-print-area">
      ...
      <div style={{
        ...
        margin: '8px 0', borderTop: '1px dashed #000', borderBottom: '1px dashed #000',
