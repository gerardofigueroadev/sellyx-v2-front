...
type WaResponseType = 'text' | 'menu' | 'order_status';
interface WaKeyword { id: number; keyword: string; responseType: WaResponseType; response: string | null; isActive: boolean; }

const RESPONSE_TYPES: { value: WaResponseType; label: string; icon: string; hint: string }[] = [
  { value: 'text',         label: 'Mensaje personalizado', icon: '💬', hint: 'Escribes el texto que responde el bot' },
  { value: 'menu',         label: 'Menú del negocio',      icon: '🍽️', hint: 'Envía automáticamente tus productos con precios' },
  { value: 'order_status', label: 'Estado de pedido',      icon: '📦', hint: 'El cliente escribe el número y recibe el estado' },
];

function WaTypeBadge({ type }: { type: WaResponseType }) {
  const t = RESPONSE_TYPES.find(r => r.value === type);
  const colors: Record<WaResponseType, string> = {
    text: 'bg-slate-600/30 text-slate-300',
    menu: 'bg-orange-600/20 text-orange-300',
    order_status: 'bg-blue-600/20 text-blue-300',
  };
  return (
    <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full ${colors[type]}`}>
      {t?.icon} {t?.label}
    </span>
  );
}
...
function TabWhatsapp({ token }: { token: string }) {
  ...
  const canAddKw = newKw.keyword.trim() !== '' && (newKw.responseType !== 'text' || newKw.response.trim() !== '');

  const addKeyword = async () => {
    if (!canAddKw) return;
    setAddingKw(true);
    try {
      const body = { keyword: newKw.keyword.trim(), responseType: newKw.responseType, response: newKw.responseType === 'text' ? newKw.response.trim() : null };
      const res = await apiFetch(token, '/whatsapp/keywords', { method: 'POST', body: JSON.stringify(body) });
      if (!res.ok) throw new Error();
  ...
  const selectedType = RESPONSE_TYPES.find(r => r.value === newKw.responseType)!;
    ...
    <div className="max-w-2xl space-y-5">
      ...
      <div className="bg-slate-800 border border-slate-700/50 rounded-2xl p-5 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-white font-bold text-sm">Conexión con WhatsApp</h3>
        ...
        <Field label="Token de acceso">
          <div className="relative">
            <input
              ...
              placeholder="EAALbVo..."
              className="w-full bg-slate-700/60 border border-slate-600 rounded-xl px-4 py-2.5 text-white text-sm placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-green-500 pr-16"
            />
        ...
        <Field label="Token de verificación (el que pusiste en Meta)">
      ...
      <div className="bg-slate-800 border border-slate-700/50 rounded-2xl p-5 space-y-4">
        <div>
          ...
          <p className="text-slate-500 text-xs mt-0.5">Cuando el cliente escribe una palabra clave, el bot responde automáticamente</p>
        </div>

        {keywords.length === 0 && (
          <p className="text-slate-500 text-sm text-center py-4">Aún no tienes respuestas configuradas</p>
        )}

        {keywords.map(kw => (
          <div key={kw.id} className={`flex items-start gap-3 p-3 rounded-xl border transition ${kw.isActive ? 'border-slate-600 bg-slate-700/30' : 'border-slate-700/30 bg-slate-800/30 opacity-50'}`}>
            <div className="flex-1 min-w-0 space-y-1">
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-white text-sm font-medium bg-slate-700 px-2 py-0.5 rounded-lg">"{kw.keyword}"</span>
                <WaTypeBadge type={kw.responseType} />
              </div>
              {kw.responseType === 'text' && kw.response && (
                <p className="text-slate-400 text-xs line-clamp-2 pl-1">{kw.response}</p>
              )}
              {kw.responseType === 'menu' && (
                <p className="text-slate-500 text-xs pl-1 italic">Enviará el menú actual del negocio con precios</p>
              )}
              {kw.responseType === 'order_status' && (
                <p className="text-slate-500 text-xs pl-1 italic">El cliente escribe el número de pedido y recibe el estado</p>
              )}
            </div>
            <div className="flex items-center gap-1 shrink-0">
              <button onClick={() => toggleKeyword(kw)}
                className={`text-xs px-2 py-1 rounded-lg transition ${kw.isActive ? 'bg-green-600/20 text-green-400 hover:bg-green-600/30' : 'bg-slate-600/20 text-slate-400 hover:bg-slate-600/30'}`}>
                {kw.isActive ? 'Activa' : 'Inactiva'}
              </button>
              <button onClick={() => deleteKeyword(kw.id)}
                className="text-red-400 hover:text-red-300 text-xs px-2 py-1 rounded-lg hover:bg-red-600/10 transition">
                Eliminar
              </button>
            </div>
          </div>
        ...
        <div className="border border-dashed border-slate-600 rounded-xl p-4 space-y-3">
          <p className="text-slate-300 text-xs font-semibold">+ Nueva respuesta automática</p>

          <div className="grid grid-cols-1 gap-2">
            <input value={newKw.keyword} onChange={e => setNewKw(p => ({ ...p, keyword: e.target.value }))}
              placeholder='Palabra clave (ej: menu, horario, pedido...)'
          ...
          <div className="grid grid-cols-3 gap-2">
            {RESPONSE_TYPES.map(rt => (
              <button key={rt.value} onClick={() => setNewKw(p => ({ ...p, responseType: rt.value }))}
                className={`flex flex-col items-center gap-1 p-2 rounded-xl border text-center transition ${newKw.responseType === rt.value ? 'border-green-500 bg-green-600/10 text-white' : 'border-slate-600 bg-slate-700/30 text-slate-400 hover:border-slate-500'}`}>
          ...
          <p className="text-slate-500 text-xs">{selectedType.icon} {selectedType.hint}</p>

          {/* Campo de texto solo si es tipo text */}
          {newKw.responseType === 'text' && (
            <textarea value={newKw.response} onChange={e => setNewKw(p => ({ ...p, response: e.target.value }))}
              rows={3} placeholder='Escribe la respuesta que verá el cliente...'
              className="w-full bg-slate-700/60 border border-slate-600 rounded-lg px-3 py-2 text-white text-sm placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-green-500 resize-none" />
          )}

          <button onClick={addKeyword} disabled={addingKw || !canAddKw}
            className="w-full bg-green-600 hover:bg-green-500 disabled:bg-slate-700 disabled:text-slate-500 text-white py-2 rounded-lg text-sm font-medium transition flex items-center justify-center gap-2">
            {addingKw && <span className="animate-spin w-3 h-3 border-2 border-white border-t-transparent rounded-full inline-block" />}
            Agregar respuesta
          </button>
        </div>
      </div>
